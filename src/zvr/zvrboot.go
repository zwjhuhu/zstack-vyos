...
func startZvr() {
	b := utils.Bash{
		Command: "bash -x /etc/init.d/virtualrouteragent restart >> /tmp/agentRestart.log 2>&1",
	}
	b.Run()
	b.PanicIfError()
}
...
func main() {
	...
	configureVyos()
	startZvr()
	log.Debugf("successfully configured the sysmtem and bootstrap the vrouter virtual router agents")
}
